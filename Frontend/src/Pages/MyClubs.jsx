...
//     setIsLoading(true);
//     try {
//       const { data } = await axios.get("http://localhost:3000/api/clubs/", {
...
//               <div
//                 key={club._id}
//                 className="group relative p-6 bg-white/80 backdrop-blur-sm rounded-3xl border border-blue-100 hover:bg-white transition-all duration-300"
//               >
...
//                   <div className="mb-6">
//                     <img
//                       src={`http://localhost:3000/${club.logo?.replace(
//                         /\\/g,
//                         "/"
//                       )}`}
//                       alt={`${club.name} Logo`}
...
//                     onClick={() => navigate(`/club/${club._id}`)} // Use navigate instead of history.push
//                   >
//                     View Details
//                   </button>
//                 </div>
...
//               You are not affiliated with any clubs yet.
//             </div>
//           )}
//         </div>
//       </div>
...
const MyClubs = () => {
  ...
  // Fetch clubs based on user authentication
  const getUserClubs = async () => {
    setIsLoading(true);
    try {
      const { data } = await axios.get("http://localhost:3000/api/clubs/my-clubs", {
        withCredentials: true,
      });
      // Since the endpoint returns the clubAffiliations array directly
                  ...
                  <div className="mb-6">
                    {club.logo && (
                      <img
                        ...
                        className="h-40 w-40 rounded-2xl object-cover"
                        onError={(e) => {
                          e.target.src = "/placeholder-club-logo.png"; // Add a placeholder image
