...
const Navbar = () => {
  ...
  const { logout, user } = useContext(AuthContext); // Get user role from context
  ...
  // Show only first 3 links for Club Admin, all links for others
  const navItems = user?.role === 'clubAdmin' ? allNavItems.slice(0, 3) : allNavItems;  // Added role wise navbar 
